...
Future<LNURLPageResult?> handleAuthRequest(BuildContext context, LnUrlAuthRequestData reqData) async {
  return promptAreYouSure(context, body: LoginText(domain: reqData.domain)).then((bool? permitted) async {
...
void handleLNURLAuthPageResult(BuildContext context, LNURLPageResult result) {
  if (result.hasError) {
    ...
    promptError(context, title: context.texts().lnurl_webview_error_title, body: Text(result.errorMessage));
    throw result.error!;
  }
}
